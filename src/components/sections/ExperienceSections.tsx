  ...
  return (
    <section id="modes" className="relative z-10 bg-dark px-3 py-16 sm:px-5 sm:py-20 md:px-6 md:py-28">
      <div className="mx-auto max-w-[1500px] px-2 sm:px-4 md:px-6">
        <FadeIn y={30}><div className="flex flex-col gap-5 border-b border-white/10 pb-8 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] uppercase tracking-[0.35em] text-[#5EEAD4]">THREE WAYS TO WORK</p><h2 className="mt-3 text-4xl font-black uppercase leading-none text-mist sm:text-6xl md:text-7xl">Not another app.<br />A new operating mode.</h2></div><p className="max-w-md text-sm leading-7 text-mist/45">Violoop meets the computer where the work already happens—then changes how much of that work needs your hands.</p></div></FadeIn>

        <div className="mt-8 grid gap-6 lg:grid-cols-[320px_1fr]">
          <div className="flex flex-col gap-2">{modes.map((item, index) => <button key={item.id} type="button" onClick={() => setActive(index)} className={`rounded-2xl border px-5 py-5 text-left transition ${active === index ? 'border-[#15B8A6]/60 bg-[#15B8A6]/12 text-mist shadow-[0_0_36px_rgba(21,184,166,.16)]' : 'border-white/10 bg-white/[0.02] text-mist/40 hover:border-white/20 hover:text-mist/70'}`}><p className="text-[10px] tracking-[0.2em]">{item.label}</p><p className="mt-2 text-lg font-semibold leading-tight">{item.title}</p></button>)}</div>
          ...
          <AnimatePresence mode="wait">
            <motion.article key={mode.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.28 }} className="grid overflow-hidden rounded-[28px] border border-white/10 bg-[#08070b] md:grid-cols-[1.15fr_.85fr]">
              <div className="relative min-h-[300px] overflow-hidden md:min-h-[480px]">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-[#08070b] via-transparent to-[#0B7F75]/10" />
              ...
              <div className="flex flex-col justify-center p-6 sm:p-8 md:p-10">
                <p className="text-[10px] uppercase tracking-[0.25em] text-[#5EEAD4]">{mode.label}</p><h3 className="mt-4 text-3xl font-black uppercase leading-[1.02] text-mist">{mode.title}</h3><p className="mt-5 text-sm leading-7 text-mist/50">{mode.copy}</p>
                <ul className="mt-7 space-y-3 border-t border-white/10 pt-6">{mode.facts.map((fact) => <li key={fact} className="flex items-center gap-3 text-sm text-mist/65"><span className="h-1.5 w-1.5 rounded-full bg-[#5EEAD4] shadow-[0_0_10px_#5EEAD4]" />{fact}</li>)}</ul>
              </div>
            </motion.article>
          </AnimatePresence>
        </div>
...
export function PricingSection() {
  return (
    <section id="pricing" className="relative z-10 bg-dark px-3 py-16 sm:px-5 sm:py-20 md:px-6 md:py-28">
      <div className="mx-auto max-w-[1500px] px-2 sm:px-4 md:px-6">
        <FadeIn y={30}><div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between"><div><p className="text-[10px] uppercase tracking-[0.35em] text-[#5EEAD4]">EARLY ACCESS STRATEGY</p><h2 className="mt-4 text-5xl font-black uppercase leading-none text-mist sm:text-7xl">Enter early.<br />Keep the advantage.</h2></div><p className="max-w-sm text-sm leading-7 text-mist/45">The reservation is designed to hold founder access without charging the full product price today.</p></div></FadeIn>
        <div className="mt-10 grid gap-4 lg:grid-cols-3">{pricing.map((tier) => <article key={tier.label} className={`relative flex flex-col overflow-hidden rounded-[28px] border p-6 sm:p-8 ${tier.highlight ? 'border-[#15B8A6]/65 bg-[radial-gradient(circle_at_80%_0%,rgba(45,212,191,.2),transparent_35%),linear-gradient(145deg,rgba(11,127,117,.22),rgba(21,184,166,.08))] shadow-[0_0_50px_rgba(13,148,136,.18)]' : 'border-white/10 bg-white/[0.02]'}`}>{tier.highlight && <span className="absolute right-5 top-5 rounded-full bg-[#15B8A6] px-3 py-1 text-[9px] font-semibold uppercase tracking-[0.16em] text-white">BEST ENTRY</span>}<p className="text-[10px] uppercase tracking-[0.24em] text-[#5EEAD4]">{tier.label}</p><div className="mt-8 flex items-end gap-3"><span className="text-6xl font-black leading-none text-mist">{tier.price}</span><span className="pb-1 text-[10px] uppercase tracking-wider text-mist/30">{tier.suffix}</span></div><h3 className="mt-6 text-2xl font-bold text-mist">{tier.headline}</h3><p className="mt-3 min-h-16 text-sm leading-6 text-mist/45">{tier.copy}</p><ul className="mt-6 space-y-3 border-t border-white/10 pt-6">{tier.points.map((point) => <li key={point} className="flex items-center gap-3 text-sm text-mist/60"><Check size={15} className="text-[#5EEAD4]" />{point}</li>)}</ul>{tier.highlight && <div className="mt-8"><ContactButton label="RESERVE FOR $10" className="w-full" /></div>}</article>)}</div>
