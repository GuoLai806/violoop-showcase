...
const SCENE_CONTROLS = [
  { event: '侧视图', label: 'SIDE' },
  { event: '顶视图', label: 'TOP' },
  { event: '热力', label: 'HEAT' },
  { event: '点位', label: 'POINTS' },
] as const
...
function shouldPreloadScene() {
  ...
  return params.has('web3d') || window.location.hash === '#products'
}
...
export default function Web3dSceneViewer({
  ...
  return (
    <div ref={hostRef} className={`relative h-full w-full bg-[#070707] ${className}`}>
      ...
      {showLoading ? (
        <LoadingOverlay
          label={loadPhase === 'scene' ? 'INITIALIZING 3D SCENE…' : 'LOADING 3D ASSETS…'}
        />
      ) : null}
      ...
      {status === 'error' ? (
        <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center bg-[#070707]/90 px-4 text-center text-sm text-mist/55">
          The 3D scene could not be loaded. Please refresh and try again.
        </div>
      ) : null}
